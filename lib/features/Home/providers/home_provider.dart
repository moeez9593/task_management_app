...
class HomeNotifier extends Notifier<HomeState> {
  ...
  void filterTaskList () 
  {
    ...
    {
      final tasks = taskList.where((task)=>task.priority==Priority.high).toList(); 
      ref.read(taskProvider.notifier).setTaskList(tasks);
    }
    else if (state.filterValue==2)
    {
      final tasks = taskList.where((task)=>task.priority==Priority.medium).toList(); 
      ref.read(taskProvider.notifier).setTaskList(tasks);
    }
    else if (state.filterValue==3)
    {
      final tasks = taskList.where((task)=>task.priority==Priority.low).toList(); 
      ref.read(taskProvider.notifier).setTaskList(tasks);
    }
    else 
    {
